...
const Header = () => {
    ...
    return (
        <header>
            <nav className='md:px-[40px] lg:px-10 px-5 md:py-8 py-4 flex items-center backdrop-blur fixed top-0 left-0 w-full z-50 bg-navy bg-opacity-80'>
                ...
                <ul className='navbar-nav ml-auto hidden font-mono md:flex'>
                    {menu && menu.map((item) => (
                        <li key={item.id} className='mx-1.5'>
                            <a data-id={`${item.id}.`} href={item.tagID} className={`tracking-widest inline-block p-2.5 capitalize before:content-[attr(data-id)] text-sm before:mr-1.5 before:text-green text-lightest-slate hover:text-green focus:text-green transition-all`}>{item.name}</a>
                        </li>
                    ))}
                </ul>
                <div className='cv ml-4 md:block hidden'>
                    <a href='/' className='button small-button'>resume</a>
                </div>
                <MobileMenu />
            </nav>
        </header>
